...
- A **stage** is `NS = 18` strokes; a **stroke** is `NP = 12` points (`Point = [x, y]` in the `1600×900` world). Constructors build strokes: `seg` (line), `ell` (ellipse), `circle` (an `ell` with equal radii), `poly` (arc-length-resampled polyline), `collapse` (all 12 points parked at one spot — an "unused" stroke). `extent(pts)` returns a stroke's bounding-box width + height — the engine's test for "is this stroke really there".
...
- **Slots are the contract.** Adjacent stages morph stroke-for-stroke, so stroke slot `i` in one stage becomes slot `i` in the next. A slot is a pen stroke's *identity*, not its meaning — what it depicts changes freely: slot 5 is an arm in stage 0, the tie body in stage 4, a steam squiggle in stage 5; slot 2 is an eye in the figure stages and the coffee surface in stage 5. That is why the engine keys fills on slot **and** stage, never on slot alone. A slot a stage doesn't use parks at a `collapse` point. Never reorder a stage's strokes without matching every other stage.
...
- **Boil (idle wobble):** every `BOIL_MS` (160) a fresh `NS×NP` grid of jitter in `[−1.75, +1.75]` is applied; scaled down for small strokes (`jScale = min(1, extent(pa)/110)`) and tapered to 0 at endpoints (`sin(π·i/(n−1))`) so joints stay joined. Skipped under reduced-motion.
...
`ResumeLink` is deliberately **not** in this catalog — it is static, carries no marker, and the engine never looks for it.
