...
Verify all four render cleanly: **mobile portrait** (e.g. 390×844), **phone landscape** (844×390 — the `stageH < 480` branch), **desktop** (1920×1080), and **ultrawide** (3440×1440 — the 500px cap holds). No layout should need a CSS breakpoint.
...
- `onScroll` keeps `window.innerHeight` — the scroll range (`scrollHeight − innerHeight`) is a document question, not a stage one. It is the one place the window is still the right source; layout is not.
