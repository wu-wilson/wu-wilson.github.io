...
wilsonwu.io is Wilson Wu's personal portfolio: a single-page, fully static, frontend-only site that plays like a short animated story on **graph paper**. One hand-drawn doodle at a time morphs into the next as the user scrolls, telling an arc in seven stages — **hello → krawly → tallies → rampr → work → coffee → mail** — with a minimal one-line caption under each drawing. There is no backend, no nav, no buttons, no footer: scrolling is the only interaction with the story, contact is a `mailto:` link, and every drawing is generated SVG (no images, no icon fonts). The single exception is a persistent **resume link** scribbled in the page's top margin — the one fixture that isn't part of the story, and the one the engine never touches. One handwriting font (Gloria Hallelujah), warm paper, school-blue grid, ink strokes, one teal accent for links.
...
- **Responsive lives in the engine, not in CSS breakpoints.** Each frame the engine derives two screen bands (drawing on top, caption below) from the stage's measured box, caps the drawing at 500px, and — for phone landscape (`stageH < 480 && stageW > stageH`) — splits to drawing-left / caption-right. There are no media queries for layout; the math is resolution-independent. Static fixtures (the resume link) stay put in every orientation and are kept clear of the bands by fluid `clamp()` insets plus a height-aware caption type scale — not by breakpoints.
...
- `.claude/skills/design-tokens/` — Exact color tokens, the font, the type scale, the graph-paper grid, and animation timing.
